...
. Addition using prompt
  ...
  - Log using console.log [numA * numB]
*/
let numA=prompt(`Enter a number`), numB=prompt(`Enter Second Number`);
alert(`Sum=${Number(numA)+Number(numB)}`);
alert(`Difference=${Number(numA)-Number(numB)}`);
alert(`Product=${Number(numA)*Number(numB)}`);
console.log(`Difference=${Number(numA)-Number(numB)}`);
